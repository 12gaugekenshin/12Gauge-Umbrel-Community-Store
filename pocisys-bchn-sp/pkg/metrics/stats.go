...
// AcceptedShare is a bounded, live record used by the PoCiSys dashboard.
// It intentionally contains no raw headers, credentials, or full submissions.
...
type Stats struct {
	...
	recentShares map[string][]AcceptedShare
	mu           sync.RWMutex
	startedAt    time.Time
}
...
func NewStats() *Stats {
	return &Stats{
		...
		recentShares: make(map[string][]AcceptedShare),
		startedAt:    time.Now(),
	}
}
...
func (s *Stats) RecordShare(symbol string, result ShareResult, workerName string, shareDiff float64) {
	...
	if result == ShareValid {
		s.mu.Lock()
		shares := append(s.recentShares[symbol], AcceptedShare{
			WorkerName: workerName,
			Difficulty: shareDiff,
			AcceptedAt: time.Now().UTC(),
		})
		if len(shares) > maxRecentShares {
			shares = shares[len(shares)-maxRecentShares:]
		}
		s.recentShares[symbol] = shares
		s.mu.Unlock()
	}
...
func (s *Stats) getOrCreateWorker(symbol, workerName string) *WorkerStats {
	...
	if len(s.workers[symbol]) >= maxTrackedWorkers {
		var oldestName string
		var oldestTime time.Time
		for name, candidate := range s.workers[symbol] {
			candidate.mu.Lock()
			last := candidate.LastShareTime
			candidate.mu.Unlock()
			if oldestName == "" || last.Before(oldestTime) {
				oldestName, oldestTime = name, last
			}
		}
		delete(s.workers[symbol], oldestName)
	}
	ws := &WorkerStats{}
...
// GetRecentShares returns at most the newest limit accepted shares.
func (s *Stats) GetRecentShares(symbol string, limit int) []AcceptedShare {
	if limit <= 0 || limit > maxRecentShares {
		limit = maxRecentShares
	}
	s.mu.RLock()
	defer s.mu.RUnlock()
	shares := s.recentShares[symbol]
	if len(shares) > limit {
		shares = shares[len(shares)-limit:]
	}
	result := make([]AcceptedShare, len(shares))
	copy(result, shares)
	return result
}
