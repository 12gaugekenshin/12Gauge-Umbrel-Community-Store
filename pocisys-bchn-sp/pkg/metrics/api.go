...
func (a *APIServer) Start() error {
	...
	mux.HandleFunc("/api/v1/shares", a.handleShares)
...
func (a *APIServer) handleShares(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodGet {
		http.Error(w, "method not allowed", http.StatusMethodNotAllowed)
		return
	}
	resp := make(map[string][]AcceptedShare)
	for symbol := range a.stats.GetAllStats() {
		resp[symbol] = a.stats.GetRecentShares(symbol, 50)
	}
